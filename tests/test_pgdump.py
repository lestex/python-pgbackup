...
def test_dump_call_pg_dump(mocker):
	"""
	Utilize pg_dump to interact wuth Database
	"""
	proc = mocker.Mock()	
...
def test_dump_handles_oserror(mocker):
	"""
	pgdump.dump return an error if pg_dump isn't installed
	"""
...
def test_dump_file_name_with_timestamp():
	"""
	pgdump.dump_file_name returns the name of database 
	with the timestamp
	"""
	timestamp = "2018-12-03T13:14"
	assert pgdump.dump_file_name(url, timestamp) == "db_one-2018-12-03T13:14.sql"
